...
It takes a DOM element.

> DOM element: An `Element` object representing a HTML element.  
> `Element` is the most general base class from which all element objects (i.e., objects that represent elements) in a `Document` inherit.  
> Go to: [Element](https://developer.mozilla.org/en-US/docs/Web/API/Element)

### Return Value:

`createRoot` returns an object with two methods: `render` and `unmount`.

-   With above code, the element with the id of 'root' is the root of our react app.
-   So, now the div#root is the entry point of our React App i.e. react will put our entire code into this div.
